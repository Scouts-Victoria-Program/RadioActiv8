...
function dynamic_form_update(destination_toggle = false) {
  ...
  if (!url) {
    url = jQuery("#ajax_form_url").attr("data-ajax-url");
  }
  jQuery.ajax({
    url: url,
    data: {
    ...
    success: function (data) {
...

      {
        ...
        for (var i = 0; i < data.location_options.length; i++) {
          ...
          if (loc.current_session) current_session_locations += this_location;
...

      {
        ...
        if (
          (data.intelligence_options.unused.length == 0 && data.intelligence_options.used.length == 0) ||
          (data.intelligence_options.unused.length == undefined && data.intelligence_options.used.length == undefined)
        ) {
        ...
        if (data.intelligence_options.random) random_intelligence = Math.floor(Math.random() * data.intelligence_options.unused.length);
        else random_intelligence = -1;
...

      {
        ...
        if (expected_location.id && expected_location.id != -1 && actual_location_id === "") {
        ...
        if (expected_location.id != -1 && !(actual_location_id === "") && expected_location.id != actual_location_id) {
