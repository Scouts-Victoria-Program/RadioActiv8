...
def index(request):
    ...
    context["session_set_form"] = SessionListForm(
        initial={"session_list_field": ra8_session}
    )
...
@staff_member_required
def patrol_locations(request):
    # template_name = 'RadioActiv8/event/create.html'
    template_name = "RadioActiv8/master/patrol_locations.html"
    # form_class = EventForm
    # success_url = reverse_lazy("RadioActiv8:EventCreate")
    context = {}
...
def SetWorkingSession(request):
    ...
    if request.method == "POST":
        ...
        if session_form.is_valid():
            # process the data in form.cleaned_data as required
            request.session["ra8_session"] = session_form.cleaned_data[
                "session_list_field"
            ].id
            # redirect to a new URL:
            return HttpResponseRedirect(request.META.get("HTTP_REFERER"))
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if request.POST["patrol"]:
                ...
                patrol.gps_tracker = GPSTracker.objects.get(
                    id=request.POST["gpstracker"]
                )
                patrol.save()
                messages.success(
                    request, f"Assigned tracker {gpstracker} to patrol {patrol}"
                )
            else:
                messages.success(
                    request,
                    f"Unassigned tracker {gpstracker} from patrol {current_patrol}",
                )
...
def base_test(request, base_id):
    ...
    return render(
        request,
        "RadioActiv8/base/detail.html",
        {
            "base": base,
            "form_test": form,
            "patrol_form": patrol_form,
...
def event_ajax(request):
    ...
    response = {
        "patrol_options": [],
        "location_options": [],
        "intelligence_options": {"unused": {}, "used": {}},
        "valid_destinations": {"unvisited": {}, "visited": {}},
        "base_history": {"visited_bases": [], "last_destination": None},
    }
    ...
    response["valid_destinations"] = valid_next_base_options(
    ...
    if request.method == "POST":
        ...
        if session_form.is_valid():
            ...
            event = Event(
                session=session,
                patrol=patrol,
                location=session.home_base,
                intelligence_answered_correctly=False,
                destination=base,
                comment=f"Assigned tracker {gps_tracker}",
            )
            event.save()
            messages.success(
                request,
                f"Patrol {patrol} added to session {session} allocated tracker {gps_tracker}, sent to base {base}",
            )
            # redirect to a new URL:
            return HttpResponseRedirect(
                reverse("RadioActiv8:SessionAddPatrol", args=[pk])
            )
    # if a GET (or any other method) we'll create a blank form
    else:
        session_add_patrol_form = SessionAddPatrolForm(
            session=this_session, initial={"ra8_session": pk}
        )
...
def participant_homepage(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            return HttpResponseRedirect(
                reverse(
                    "RadioActiv8:participant_base_list", args=(request.POST["patrol"],)
                )
            )
        else:
            context["form"] = form
...
def participant_base_list(request, pk):
    ...
    context["bases"] = (
        Base.objects.filter(is_home_base=None)
        .filter(session__in=patrol.session.all())
        .exclude(activity_type="F")
    )
...
def valid_intelligence_options(patrol, current_location):
    ...
    used_options = current_location.radio.base.get_intelligence().exclude(
        id__in=unused_options
    )

    response["unused"] = [
        {"id": o.id, "q": o.question, "a": o.answer} for o in unused_options
    ]
    response["used"] = [
        {"id": o.id, "q": o.question, "a": o.answer} for o in used_options
    ]
...
def patrol_base_history(session, patrol):
    ...
    response = {
        ...
        "last_destination": last_destination_response,
    }
